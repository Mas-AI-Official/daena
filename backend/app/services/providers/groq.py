"""Groq provider adapter — fast inference.

Groq provides extremely fast inference via custom LPU hardware.
Uses OpenAI-compatible Chat Completions API format.

Pricing: cheapest cloud option after Ollama.
"""
...
_API_BASE = "https://api.groq.com/openai"
...
class GroqProvider(BaseProvider):
    """Groq fast-inference provider (OpenAI-compatible API)."""

    def __init__(self, api_key: str | None = None, timeout: float = 60.0) -> None:
        super().__init__(ModelProvider.GROQ)
    ...
    def _find_model(self, model_id: str) -> ModelInfo:
        ...
        return ModelInfo(
            ...
            cost_per_1m_input=0.59, cost_per_1m_output=0.79,
        )
