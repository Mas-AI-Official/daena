...
# Legacy UI routes: redirect to main UI (templates consolidated: approval_workflow -> control-panel, cmp_canvas/control_center -> cmp-voting/command-center)
@router.get("/approvals", response_class=HTMLResponse)
async def serve_approvals():
    """Redirect to Control Pannel approvals section"""
    return RedirectResponse(url="/ui/control-pannel#tasks-runbook-approvals", status_code=302)
...
async def serve_cmp_canvas():
    """Redirect to CMP Voting UI"""
    return RedirectResponse(url="/ui/cmp-voting", status_code=302)


@cmp_canvas_router.get("/control-center", response_class=HTMLResponse)
async def serve_control_center():
    """Redirect to Daena Command Center"""
