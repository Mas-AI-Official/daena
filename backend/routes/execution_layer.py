...
async def execution_auth_status(request: Request) -> Dict[str, Any]:
    """
    Public endpoint (no X-Execution-Token). Tells the UI whether the Execution Layer
    is enabled, whether localhost bypass is active, and what the UI should show
    (paste token vs dev mode vs disabled).
    """
    execution_enabled = bool(settings.execution_token)
    localhost_bypass = getattr(settings, "allow_insecure_execution_local", False) and _is_localhost_request(request)
    if not execution_enabled:
        message = "Execution Layer is disabled. Set EXECUTION_TOKEN in .env to enable."
    elif localhost_bypass:
        message = "Local dev: from 127.0.0.1 token is optional (ALLOW_INSECURE_EXECUTION_LOCAL=1)."
    else:
        message = "Paste the same EXECUTION_TOKEN (from backend .env) below for this session to call tools/config."
    return {
        "success": True,
