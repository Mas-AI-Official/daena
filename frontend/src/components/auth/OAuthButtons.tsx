 ...
 *
 * Flow: Click → GET /auth/oauth/{provider}/authorize → get URL → redirect.
 * After provider consent, backend redirects to /auth/callback with exchange code.
 */

type Provider = 'google' | 'github'

const PROVIDER_CONFIG: Record<Provider, { label: string; icon: ReactNode; bg: string }> = {
  google: {
    label: 'Google',
...
export function OAuthButtons() {
  ...
  const handleOAuth = async (provider: Provider) => {
    ...
    try {
      ...
      // If OAuth not configured, provider will return error — let it fail silently
